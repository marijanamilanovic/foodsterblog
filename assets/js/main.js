...
function fetchNav(){
    $.ajax({
        ...
        error: function(xhr){
            $("#navigation").html(JSON.parse(xhr.responseText));
        }
    })
}
...
function printLatestPosts(data){
    ...
    for(let d of data){
        ...
        <a href="recipe.php?id=${d.idPost}" class="blog-article-post height-full bg-color-three d-flex flex-wrap align-content-between">
...
function printCategories(data){
    let print = "";
    data.forEach(d => {
        print += `<a href="recipes.php?page=1&category=${d.idCategory}" class="col-lg-4 bg-color-three mt-lg-0 mt-0">
...
function vote(){
    var rb = $('input[name="answer"]:checked');
    var idSurvey = $("#idSurvey").val();
    var choice = rb.val();

    if(choice != undefined){
        $("#surveyError").html("Thank you for voting!");
        $.ajax({
            url: "model/fillSurvey.php",
            method: "post",
            dataType: "json",
            data: {
                "choice": choice,
            ...
            success: function(data){
                $("#surveyError").html(data);
                $('input[name="answer"]').attr("disabled", true);
                results();
            },
            error: function(xhr){
                $("#surveyError").html(JSON.parse(xhr.responseText));
            }
        })
    }
    else{
        $("#surveyError").html("You didn't choose an answer.");
    }
}
...
function results(){
    ...
    $.ajax({
        ...
        success: function(data){
            let print = "";
            for(let d of data){
                print += `<p>${d.answerText}: ${d.count} votes.</p>`;
            }
            $("#printResults").html(print);
        },
        error: function(xhr){
            $("#printResults").html(JSON.parse(xhr.responseText));
        }
    })
}
...
function admin(){
    ...
    $("#adminPanel a").click(function(e){
        ...
        if(choice == "allSurveys"){
            allSurveys();
        }
        if(choice == "addSurvey"){
            addSurvey();
        }
        if(choice == "activeSurveys"){
            activeSurveys();
        }
        if(choice == "allUsers"){
            allUsers();
    ...
    $.ajax({
        ...
        success: function(survey){
            ...
            print += `<div id="table">
                        ...
                        <ul>`;
            for(let s of survey){
                print += `<li>${s.question} ${s.answerText} (${s.count} votes)</li>`;
            }
            print += `</ul></div>`;
            $("#recipes").html(print);
        },
...
function activeSurveys(){
    $.ajax({
        url: "model/changeActivitySurvey.php",
        method: "post",
        dataType: "json",
        success: function(survey){
            let print = "";
            print += `<div id="table">
                        ...
                        <form>
                            <select id="activeSurvey">`;
            for(let s of survey){
                print += `<option value="${s.idSurvey}"`;
                if(s.active == 1){
                    print += `selected`;
                }
                print += `>${s.question}</option>`;
            }
            print += "</select></form><div id='errorAdmin'></div></div>";
            $("#recipes").html(print);
            $("#activeSurvey").change(changeActivity);
        },
        error:function(xhr){
            $("#recipes").html(xhr);
        }
...
function addSurvey(){
    let print = `<div class="updateDiv">
                    <h2>Add new survey</h2>
                    <div id="formUpdate">
                        <form>
                            <input type="text" placeholder="Question" id="question"/>
                            <input type="text" placeholder="Answer 1" name="answer[]"/>
                            <input type="text" placeholder="Answer 2" name="answer[]"/><br/>
                            <div id="newQuestions"></div>
                            <input type="button" value="Add answer" id="addAnswer"/>
                            <input type="button" value="Add survey" id="addNewSurvey"/>
                            <div id="errorAdmin"></div>
                        </form>
                    </div>
                </div>`;
    ...
    $("#addAnswer").click(function(){
        let print2 = `<input name="answer[]" type="text" placeholder="Answer ${count}"><br/>`;
        count++;
        document.getElementById("newQuestions").innerHTML += print2;
    })
    $("#addNewSurvey").click(insertSurvey);   
}
...
function insertSurvey(){
    ...
    for(let i=0; i<answer.length; i++){
        dataToSend.push(answer[i].value);
    }
    $.ajax({
        url: "model/addSurvey.php",
        method: "post",
        dataType: "json",
        data: {
            "question": question,
